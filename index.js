...
const vcard = 'BEGIN:VCARD\n'
            ...
            + 'FN:lazaro\n' // Nama kamu
            + 'ORG:Lexa;\n' // Nama bot
            + 'TEL;type=CELL;type=VOICE;waid=558896657187:+55 8896657187\n' //Nomor whatsapp kamu
            + 'END:VCARD'
prefix = '.','!','#'
blocked = []
limitawal = '999999999'
...
async function starts() {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			const ownerNumber = ["558896657187@s.whatsapp.net"] // replace this with your number
			const mod = [ownerNumber,"558896657187@s.whatsapp.net"]//ubah nomor lo
			const adminbotnumber = ["558896657187@s.whatsapp.net"]//ubah nomor lo
			const frendsowner = ["558896657187@s.whatsapp.net"]//ubah nomor lo
			const premium = ["558896657187@s.whatsapp.net","558589573335@s.whatsapp.net","5511975987275@s.whatsapp.net"]
...
break
		   ...
		   await limitAdd(sender)
				...
				case 'addfoto':
					if (!isGroup) return reply(mess.only.group)
					if (!isOwner) return reply('Você quem é o don.o? ')
...
break
				...
				case 'wibu':
                   ...
                   break
					...
					break
				case 'desligar':
				if (!isOwner) return reply('Comando so para o meu don.o, qm e vc?')
...
// recursos anime, random @Causs domina vida
          ...
          reply('Espere..')
			    ...
			    case 'owner':
                    ...
                    client.sendMessage(from, 'Ctt do meu don.o ai, pfv n flode o chat',MessageType.text, { quoted: mek} )
                    break
                case 'fitnah':	
				case 'fake':          
                    if (!isGroup) return reply(mess.only.group)
				...
				case 'info':
					me = client.user
					uptime = process.uptime()
					teks = `*Nome do bot* : ${me.name}\n*Número do bot* : @${me.jid.split('@')[0]}\n*Prefixo* : ${prefix}\n*Contatos bloqueados* : ${blocked.length}\n*O bot está ativo em* : ${kyun(uptime)}\n\n*Digite .dono para ver a info do d.ono*`
           ...
           case 'shitlist':
               ...
               break
				...
				case 'dono':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '*CRIADOR:* LAZARO\n*YOUTUBE:* https://youtube.com/channel/UCt0LFC4oCZASYGsMB3eTVqw\n*WPP:* wa.me/+558896657187\n*INSTA:* @lazaro_cdv\n\n\nEspero que tenham gostado do bot 🐊🚩️'})
					break
				case 'setprefix':
					if (args.length < 1) return
					if (!isOwner) return reply(mess.only.ownerB)
			...
			reply(mess.wait)
                ...
                case 'marcar2':
                 ...
                 case 'marcar3':
					...
					break
				case 'limpar':
					if (!isOwner) return reply('Quem é Você?, Voce não é meu dono quem e meu dono e lazaro seu otario 😂')
        ...
        case 'promover':
				...
				case 'simih':
					if (!isGroup) return reply(mess.only.group)
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (args.length < 1) return reply('Hmlazarommm')
					if (Number(args[0]) === 1) {
